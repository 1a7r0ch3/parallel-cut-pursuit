...
import os 
import sys
import wrapper_cp_pfdr_d1_lsx.cp_pfdr_d1_lsx_py_C_API as cp

def cp_pfdr_d1_lsx_py(loss, Y, first_edge, adj_vertices, edge_weights=None, 
                      ...
                      verbose=int(1e2), out_Obj=False, out_Time=False, 
                      out_Dif=False):
   ...
   pfdr_it_max = 1e4, verbose = 1e2)
    ...
    separable loss term and simplex constraints: minimize functional over a 
    graph G = (V, E)
    ...
    where for each vertex, x_v is a D-dimensional vector,
          ...
          and i_{simplex} is the standard D-simplex constraint over each 
          vertex,
 ...
 splitting algorithm.
    ...
    loss_weights - weights on vertices (ignored for linear loss); (real) array
                   of length V or empty for no weights
    d1_coor_weights - for multidimensional data, weights the coordinates in the
                      l1 norms of finite differences; all weights must be 
                      strictly positive, and it is advised to normalize the 
    ...
    out_Dif  - function return the optional output Dif if set to True 
     ...
     Comp - assignement of each vertex to a component, array of length V 
            (uint16)
     rX   - values of eachcomponents of the minimizer, array of length rV 
            (real); the actual minimizer is then reconstructed as 
            X = rX(Comp + 1);
   ...
   """
    ...
    if Y.any() and Y.dtype == 'float64':
        ...
        raise TypeError("Y must be non empty numpy array of float (single or double)") 
    
    # Convert in numpy array scalar entry :Y, first_edge, adj_vertices, edge_weights, loss_weights, d1_coor_weights and define float numpy array argument with the right float type, if empty:
    if type(Y) != np.ndarray:
        raise TypeError("Y must be numpy array of float (single or double)") 
    ...
    if type(edge_weights) != np.ndarray:
        if type(edge_weights) == list:
            raise TypeError("edge_weights can not be a list, must be either a %s type or a numpy.array " %real_t)
    ...
    if type(loss_weights) != np.ndarray:
        if type(loss_weights) == list:
            raise TypeError("loss_weights can not be a list, must be of either a %s or a numpy.array" %real_t)
    ...
    if type(d1_coor_weights) != np.ndarray:
        if type(d1_coor_weights) == list:
            raise TypeError("d1_coor_weights can not be a list, must be of either a %s or a numpy.array" %real_t)
        elif d1_coor_weights != None:
            d1_coor_weights = np.array([d1_coor_weights], dtype = real_t)
        else:
            d1_coor_weights=np.array([], dtype=real_t)
 
    # Check type of all numpy.array arguments of type float (Y, edge_weights, loss_weights, d1_coor_weights) 
    for name, ar_args in zip(["Y", "edge_weights", "loss_weights", "d1_coor_weights"], [Y, edge_weights, loss_weights, d1_coor_weights]):
        if ar_args.dtype != real_t:
            raise TypeError("%s must be of %s type " %(name, real_t))

    # Check fortran continuity of all numpy.array arguments of type float (Y, first_edge, adj_vertices, edge_weights, loss_weights, d1_coor_weights)
    for name, ar_args in zip(["Y", "first_edge", "adj_vertices", "edge_weights", "loss_weights", "d1_coor_weights"], [Y, first_edge, adj_vertices, edge_weights, loss_weights, d1_coor_weights]):
    ...
    for name, b_args in zip(["out_Obj", "out_Time", "out_Dif"], [out_Obj, out_Time, out_Dif]):
        if type(b_args) != bool:
            raise TypeError("%s must be of bool type" %name)

    # Call wrapper python in C  
    Comp, rX, it, Obj, Time, Dif = cp.py_C_API_function(loss, Y, first_edge, 
        ...
        pfdr_it_max, verbose, real_t == 'float64', out_Obj, out_Time, out_Dif) 
